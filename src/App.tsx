...
export default function App() {
  const [activeTab, setActiveTab] = useState<'overview' | 'knowledge' | 'local-ingestion' | 'reasoning' | 'graph'>('overview');
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 p-6 max-w-7xl w-full mx-auto space-y-6">
        ...
        {/* Local Knowledge Ingestion Tab (Phase 6A) */}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
            <h2 className="text-base font-semibold text-white flex items-center justify-between">
              <span className="flex items-center space-x-2">
                ...
                <span>Runtime Graph State ({graphData.nodes.length} Nodes, {graphData.edges.length} Edges)</span>
              </span>
              <button
                onClick={fetchGraph}
                className="p-1.5 text-slate-400 hover:text-white bg-slate-800 rounded-lg text-xs flex items-center space-x-1"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-3">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400">Graph Nodes</h3>
                <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                  {graphData.nodes.map((node) => (
                    <div key={node.id} className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex items-start justify-between">
                      <div className="space-y-1">
                        <div className="text-xs font-semibold text-slate-200">{node.label}</div>
                        <div className="text-[10px] font-mono text-slate-500">ID: {node.id}</div>
                      </div>
                      <span className="px-2 py-0.5 text-[10px] bg-slate-900 border border-slate-700 text-emerald-400 rounded-full font-mono">
                        {node.type}
                      </span>
                    </div>
                  ))}
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400">Provenance Edges</h3>
                <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                  {graphData.edges.map((edge) => (
                    <div key={edge.id} className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
                      <div className="flex items-center justify-between text-xs text-slate-300">
                        <span className="font-mono text-indigo-400">{edge.sourceId}</span>
                        <span className="text-[10px] text-emerald-400 font-mono">--[{edge.type}]--&gt;</span>
                        <span className="font-mono text-indigo-400">{edge.targetId}</span>
                      </div>
                    </div>
                  ))}
                </div>
